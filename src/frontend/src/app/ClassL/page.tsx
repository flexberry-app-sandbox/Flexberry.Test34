...
import useGetAllClass from '@/hooks/Class/useGetAllClass';
import useDeleteAllClass from '@/hooks/Class/useDeleteAllClass';
import { ClassEnum } from '@/enums/ClassEnum.types';
import { tView } from '@/enums/tView.types';
import { IClassL } from '@/types/Class.types';
import useListState from '@/hooks/useListState';
import useExportList from '@/hooks/useExportList';
import { formatError } from '@/utils/errorsUtils';

export default function ClassPageList() {
  const viewName: string = 'ClassL';
  ...
  } = useListState<IClassL>('ClassPageList', {
    perPage: 10,
    columnWidth: {},
    columnHidden: {},
    columnOrder: {},
  ...
  const { data, isLoading, count } = useGetAllClass<IClassL>({
    viewName,
    perPage,
    page,
    sorting,
  ...
  const { deleteAllClass } = useDeleteAllClass(handleSuccess, handleError);

  const handleDelete = (items: IClassL[]) => {
    deleteAllClass(items.map((item) => item.id));
  };

  const { exportList } = useExportList({
    viewName: tView.ClassL,
    fileName: 'ClassL',
    sorting,
    filter,
    search,
